...
function todaysMeals({ compact = false, showRecipe = true } = {}) {
  const meals = [
    { time: "Breakfast", meal: "Vegetable poha", meta: "8:00 AM · 3 people", emoji: "🥣", tone: "yellow" },
    { time: "Lunch", meal: "Sambar rice", meta: "1:00 PM · 3 people", emoji: "🍛", tone: "orange" },
    { time: "Dinner", meal: "Palak paneer", meta: "8:00 PM · 4 people", emoji: "🥬", tone: "green" },
  ];
  return `
...
function cookDestination(options = {}) {
  if (state.screen === "households") return householdList({ dense: options.dense, heading: options.heading });
...
export function VariantA() {
  if (state.role === "member") {
    ...
    return phoneShell(
      `${body}${state.screen !== "chat" ? bottomNav([
        ["today", "Today", "today"],
        ["plan", "Meal Plan", "plan"],
...
export function VariantC() {
  if (state.role === "member") {
    ...
    if (state.screen === "chat") {
      return phoneShell(`${householdChatHeader({ segmented: true })}${chatBody({ pinned: true })}`, "variant-c");
    }
    const body =
      state.screen === "plan"
  ...
  const screen = state.screen === "households" ? householdList({ heading: "Households" }) : cookDestination({ segmented: true, pinned: true });
