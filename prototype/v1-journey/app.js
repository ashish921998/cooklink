...
const ACCEPTANCE = {
  ...
  F1: {
    group: 'First-use',
    text: 'A new household gets an active 7-day B/L/D plan with no approval step.',
  },
  F2: {
    group: 'First-use',
    text: 'A Cook joins only via a Household Invite; cannot self-create a household.',
  },
  // Member
  M1: { group: 'Member experience', text: 'Member fresh launch opens Today with Today selected.' },
  M2: {
    group: 'Member experience',
    text: 'Meal Plan and Groceries are one tap away; Chat is a header action, not a 4th tab.',
  },
  // Cook
  C1: {
    group: 'Cook multi-household',
    text: 'Cook fresh launch opens the Household list even with one household.',
  },
  C2: {
    group: 'Cook multi-household',
    text: 'Selecting a household opens Chat; exactly 3 persistent destinations.',
  },
  C3: {
    group: 'Cook multi-household',
    text: 'Switching households never leaks prior household state.',
  },
  // Plan editing
  P1: {
    group: 'Automatic plan editing',
    text: 'Swap, regenerate (meal/day/week), and search are available from Meal Plan.',
  },
  P2: {
    group: 'Automatic plan editing',
    text: 'Cook edits apply immediately and are attributed; no Member approval.',
  },
  P3: {
    group: 'Automatic plan editing',
    text: 'A stale plan suggestion cannot overwrite a newer meal without fresh confirmation (optimistic concurrency).',
  },
  // Recipe Guides
  R1: {
    group: 'Recipe Guides',
    text: 'Recipe Guide shows step-by-step text + on-demand voice in EN/HI, generated server-side.',
  },
  // Chat
  H1: { group: 'Chat', text: 'Send text, one photo+caption, or one voice note ≤2 min.' },
  H2: {
    group: 'Chat',
    text: 'An EN/HI message can produce a private suggestion visible only to its author.',
  },
  H3: {
    group: 'Chat',
    text: "'I need grocery' asks for the missing item; it does not create a request.",
  },
  H4: {
    group: 'Chat',
    text: 'A confirmed action appends one attributed system event; edits never reverse a confirmed action.',
  },
  H5: {
    group: 'Chat',
    text: 'Messages are editable/deletable only by their author within 15 minutes.',
  },
  // Grocery Requests
  G1: {
    group: 'Grocery Requests',
    text: 'Cook can create a request; Member sees Approve/add-to-cart; Cook sees no price or checkout.',
  },
  G2: {
    group: 'Grocery Requests',
    text: 'Either active Cook can edit/cancel a pending request; a concurrent conflict is rejected with current state shown.',
  },
  // Estimated Pantry
  E1: {
    group: 'Estimated Pantry',
    text: 'Only likely available / may be low / unknown is shown — never exact stock.',
  },
  E2: {
    group: 'Estimated Pantry',
    text: 'Delivered orders add pantry; checkout success alone does not.',
  },
  E3: {
    group: 'Estimated Pantry',
    text: 'Cart covers today + next 2 calendar days; items labelled by need timing.',
  },
  // Checkout + fallback
  I1: {
    group: 'Instamart checkout & fallback',
    text: 'No checkout without fresh Member confirmation + a server-side role check.',
  },
  I2: {
    group: 'Instamart checkout & fallback',
    text: 'An idempotency key gates every attempt; an uncertain failure is resolved via get_orders before any retry; no duplicate order.',
  },
  I3: {
    group: 'Instamart checkout & fallback',
    text: 'Cart ≥ ₹1000 or no payment method routes to the Instamart app; no deep link is promised.',
  },
  I4: {
    group: 'Instamart checkout & fallback',
    text: 'Ordering is feature-gated; production ordering is unavailable until Swiggy grants access.',
  },
  // Failures
  FL1: {
    group: 'Failures',
    text: 'An offline message stays in its household outbox, retries, and cannot leak cross-household or act before server acceptance.',
  },
  FL2: { group: 'Failures', text: 'A removed participant immediately loses chat/media access.' },
  // Accessibility + language
  A1: {
    group: 'Accessibility & language',
    text: 'Navigation is localized EN/HI; both are never shown simultaneously.',
  },
  A2: {
    group: 'Accessibility & language',
    text: '44pt targets, accessibility labels, and correct behavior at the largest text scale & 320px width.',
  },
  // Security (cross-cutting)
  S1: {
    group: 'Security (cross-cutting)',
    text: 'Authorization is enforced in the app server on every request against (user, household, role); no RLS; client checks are nav-only.',
  },
  S2: {
    group: 'Security (cross-cutting)',
    text: 'No provider secret lives in the bundle (CI grep test).',
  },
  S3: {
    group: 'Security (cross-cutting)',
    text: 'An authorization test suite proves no cross-household data access.',
  },
};
...
function mealList({ recipeOn = true } = {}) {
  const meals = [
    {
      time: { en: 'Breakfast', hi: 'नाश्ता' },
      meal: { en: 'Vegetable poha', hi: 'सब्जी पोहा' },
      meta: '8:00 AM · 3',
      emoji: '🥣',
      tone: 'yellow',
    },
    {
      time: { en: 'Lunch', hi: 'दोपहर' },
      meal: { en: 'Sambar rice', hi: 'सांबर चावल' },
      meta: '1:00 PM · 3',
      emoji: '🍛',
      tone: 'orange',
    },
    {
      time: { en: 'Dinner', hi: 'रात' },
      meal: { en: 'Palak paneer', hi: 'पालक पनीर' },
      meta: '8:00 PM · 4',
      emoji: '🥬',
      tone: 'green',
    },
...
const STAGES = [
        ...
        <div class="status-screen" style="flex:0;padding:20px 0">
          ...
          <p>${
            state.lang === 'hi'
              ...
              : 'Your Cook gets a Household Invite. They open the link and join as the Cook.'
          }</p>
        </div>
  ...
  },
  {
    ...
    render: () => `
      ...
      <div class="screen-scroll">
        <p style="font-size:11px;color:var(--muted);margin:4px 0 12px">${
          state.lang === 'hi'
            ? 'आपने दोपहर का खाना बदलने की कोशिश की। तब तक लक्ष्मी (रसोईये) ने वह बदल दिया।'
            : 'You tried to change lunch. Meanwhile Lakshmi (Cook) changed it.'
        }</p>
      ...
      const households = [
        {
          ...
          name: 'Rao Family',
          colour: 'mango',
          next: state.lang === 'hi' ? 'दोपहर · सांबर चावल' : 'Lunch · Sambar rice',
          msg: state.lang === 'hi' ? 'आज कम मिर्च' : 'Less spicy today',
          time: '8:42',
          unread: 2,
        },
        {
          ...
          name: 'Mehta Home',
          colour: 'sage',
          next: state.lang === 'hi' ? 'रात · पालक पनीर' : 'Dinner · Palak paneer',
          msg: state.lang === 'hi' ? 'माँग मंज़ूर' : 'Request approved',
          time: state.lang === 'hi' ? 'कल' : 'Yesterday',
          unread: 0,
        },
        {
          id: 'iyer',
          ...
          name: 'Iyer Household',
          colour: 'blue',
          next: state.lang === 'hi' ? 'दोपहर · नींबू चावल' : 'Lunch · Lemon rice',
          msg: state.lang === 'hi' ? 'फ़ोटो' : 'Photo',
          time: state.lang === 'hi' ? 'कल' : 'Yesterday',
          unread: 0,
        },
        {
          ...
          name: 'Khan Family',
          colour: 'rose',
          next: state.lang === 'hi' ? 'रात · चिकन पुलाव' : 'Dinner · Chicken pulao',
          msg: state.lang === 'hi' ? 'आवाज़ · 0:18' : 'Voice · 0:18',
          time: state.lang === 'hi' ? 'सोम' : 'Mon',
          unread: 0,
        },
      ];
      return `
      ...
      const items = [
        {
          id: 'coconut',
          emoji: '🥥',
          name: { en: 'Fresh coconut', hi: 'ताज़ा नारियल' },
          pkg: '1 pc',
          pantry: 'low',
          need: { en: 'needed today', hi: 'आज चाहिए' },
          src: 'request',
        },
        {
          id: 'coriander',
          emoji: '🌿',
          name: { en: 'Coriander', hi: 'धनिया' },
          pkg: '2 bunches',
          pantry: 'low',
          need: { en: 'needed today', hi: 'आज चाहिए' },
        },
        {
          id: 'oil',
          emoji: '🛢️',
          name: { en: 'Cooking oil', hi: 'खाने का तेल' },
          pkg: '1 L',
          pantry: 'unknown',
          need: { en: 'needed tomorrow', hi: 'कल चाहिए' },
        },
        {
          id: 'rice',
          emoji: '🍚',
          ...
          pkg: '5 kg',
          pantry: 'likely',
          need: { en: 'needed day after', hi: 'परसों चाहिए' },
        },
      ];
      return `
      ...
      <div class="status-screen">
        ...
        <p>${
          state.lang === 'hi'
            ...
            : "If someone is a Member in one household and a Cook in another, which entry shell does the app open? Issue 03 explicitly deferred this to 'before implementation'."
        }</p>
      </div>
        ...
        <div style="padding:6px 20px 20px">
          <p style="font-size:11px;color:var(--muted);margin:4px 0 14px">${
            state.lang === 'hi'
              ...
              : 'Green = demonstrated by the current stage. The full list is the measurable V1 acceptance bar.'
          }</p>
          ${groups
            .map(
              (g) => `
...
function wire() {
  document
    ...
    .forEach((b) => b.addEventListener('click', () => step(Number(b.dataset.step))));
  ...
  document.querySelectorAll('[data-coverage-close]').forEach((b) =>
    b.addEventListener('click', (e) => {
      if (
        e.target.hasAttribute('data-coverage-close') ||
        e.target.closest('[data-coverage-close]')
      ) {
  ...
  document
    ...
    .forEach((b) => b.addEventListener('click', () => goStage(b.dataset.jump)));
  document.querySelectorAll('[data-toggle]').forEach((b) =>
    b.addEventListener('click', () => {
      const { toggle, val } = b.dataset;
      if (toggle === 'style' || toggle === 'diet' || toggle === 'servings')
        state.onboard[toggle] = val;
