 ...
 * expire after {@link CHECKOUT_CONFIRMATION_TTL_MS} and are scoped to the
 * issuing membership. The store is intentionally simple and not durable: a
 * restart invalidates outstanding tokens (fail-closed), and the DB-backed
 * idempotency key is the real duplicate-order safety net (AC#5).
 */
...
function serializeConfirmation(confirmation: CheckoutConfirmation) {
  return {
...
function serializeProviderOrder(order: ProviderOrder) {
  return {
    id: order.id,
    status: order.status,
